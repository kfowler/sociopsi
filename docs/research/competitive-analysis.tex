...
\begin{abstract}
\noindent
This document provides a systematic comparative analysis of the Jung Agent
...
Psi theory into the Jung Agent's existing architecture.
\end{abstract}
...
The Jung Agent and MicroPsi2 share a common ancestor in D\"orner's Psi
theory but diverge significantly in implementation philosophy:

\begin{itemize}
  \item \textbf{MicroPsi2} faithfully implements the full Psi architecture
    via node nets, spreading activation, and formal modulator dynamics.
    It is a \emph{cognitive modeling framework}---general-purpose, theoretically
    pure, and environment-agnostic.
  \item \textbf{Jung Agent} takes Psi drives as a foundation and extends
...
(modulators, node nets, ReCoN scripts) where Jung Agent has experiential
...
mechanisms into the Jung Agent, not convergence.
...
The Jung Agent's drive system is directly inspired by and closely aligned
with Psi theory.

\begin{table}[H]
\centering
\small\sffamily
\caption{Drive-by-drive alignment between Jung Agent and Psi theory}
\label{tab:drive-alignment}
\begin{tabularx}{\textwidth}{l c c X}
\toprule
\textbf{Drive} & \textbf{Psi} & \textbf{Jung} & \textbf{Notes} \\
\midrule
...
Jung Agent extends with curiosity (reasonable), recognition (reasonable), and
...
\toprule
\textbf{Mechanism} & \textbf{MicroPsi2} & \textbf{Jung Agent} \\
\midrule
Demand signal &
...
Both projects emphasize embodiment, but the Jung Agent is \emph{more}
concretely embodied than MicroPsi2. Where MicroPsi2 uses abstract world
adapters, the Jung Agent maps drives directly to real hardware sensors:
...
\textbf{Current state}: The Jung Agent has no modulator layer. The
...
dialogue system uses drive states to weight archetype voices (low affiliation
$\rightarrow$ Shadow emerges). This is a partial implementation---drive
states influence \emph{which} voices speak, but not \emph{how} they speak.

\subsection{Gap 3: Node Nets / Spreading Activation}

\begin{gapbox}
\textbf{Severity: Low (for current architecture).} MicroPsi2's node nets
provide learned, associative action selection. The Jung Agent uses a static
...
\begin{gapbox}
\textbf{Severity: Medium-Low.} The Jung Agent selects flat action lists (1--3
...
The Jung Agent has several capabilities that MicroPsi2 lacks entirely:
...
The Jung Agent generates \emph{multiple simultaneous internal voices} via
...
MicroPsi2 uses node nets for all cognition. The Jung Agent uses large
language models (Cydonia 22B via Ollama) as the substrate of thought. This
enables:
...
The Jung Agent's memory system uses sentence-transformer embeddings for
semantic search and a weight-based recall system where meditation modifies
memory salience. This is more sophisticated than MicroPsi2's protocol
chain decay.
...
\subsection{What Jung Agent Should Adopt from Psi/MicroPsi2}

\begin{enumerate}
  \item \textbf{Modulator layer} (high priority): Derive arousal, resolution,
    selection threshold, and securing rate from current drive states. Use
    these to modulate LLM parameters (temperature, top-p), cycle timing,
    archetype weighting, and action verification behavior.

  \item \textbf{Emergent emotion labels} (medium priority): Map modulator
    configurations to emotion names. Display current emotion in the console
    and feed it into the LLM context. This makes the agent's emotional state
    legible without requiring discrete emotion tracking.
...
\subsection{What MicroPsi2 Could Learn from Jung Agent}
...
Divergence from Psi\\
  orthodoxy &
  Certain & None &
  Jung Agent is not a Psi implementation---it's a Psi-\emph{inspired}
  Jungian system \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[
  ...
  % --- Jung Agent column ---
  \node[font=\sffamily\small\bfseries,color=junggold!80!black] at (4,4.5) {Jung Agent};
  ...
  \node[unique] (j-llm)     at (4,-3.5) {LLM (Cydonia)};
...
Dashed red = missing in Jung Agent. Gold = unique to Jung Agent.}
...
The Jung Agent is not a MicroPsi2 clone and should not become one. Its
...
single addition would bring the Jung Agent closer to Psi theory's most
...
activation---can be left to MicroPsi2. The Jung Agent's LLM-based cognition
...
MicroPsi2 be the rigorous cognitive model, and let Jung Agent be the
